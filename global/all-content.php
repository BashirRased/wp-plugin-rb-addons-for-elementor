...
if ( ! empty( $rbelad_controls ) && is_array( $rbelad_controls ) ) {
	foreach ( $rbelad_controls as $rbelad_key => $rbelad_values ) {
		...
		switch ( $rbelad_key ) {
			...
			case 'text_3':
				$this->add_control(
					...
					array(
						...
						'ai'          => false,
						'label_block' => true,
						'default'     => ! empty( $rbelad_values['default'] ) ? $rbelad_values['default'] : esc_html__( 'Enter your title', 'rb-addons-for-elementor' ),
						'placeholder' => ! empty( $rbelad_values['placeholder'] ) ? $rbelad_values['placeholder'] : esc_html__( 'Add Your Heading Text Here', 'rb-addons-for-elementor' ),
						'condition'   => ! empty( $rbelad_values['condition'] ) ? $rbelad_values['condition'] : array(),
					)
				);
				break;
			...
			case 'textarea':
				$this->add_control(
					...
					array(
						...
						'ai'          => false,
						'default'     => ! empty( $rbelad_values['default'] ) ? $rbelad_values['default'] : esc_html__( 'Add Your Heading Text Here', 'rb-addons-for-elementor' ),
						'placeholder' => ! empty( $rbelad_values['placeholder'] ) ? $rbelad_values['placeholder'] : esc_html__( 'Add Your Heading Text Here', 'rb-addons-for-elementor' ),
						'description' => ! empty( $rbelad_values['description'] ) ? $rbelad_values['description'] : rbelad_get_allowed_html_desc(),
						'condition'   => ! empty( $rbelad_values['condition'] ) ? $rbelad_values['condition'] : array(),
					)
				);
				break;
			...
			case 'gallery':
				$this->add_control(
					...
					array(
						'label'       => ! empty( $rbelad_values['label'] )
							? $rbelad_values['label']
							: esc_html__( 'Add Images', 'rb-addons-for-elementor' ),
						'type'        => Controls_Manager::GALLERY,
						'label_block' => isset( $rbelad_values['label_block'] ) ? $rbelad_values['label_block'] : true,
						'show_label'  => isset( $rbelad_values['show_label'] ) ? $rbelad_values['show_label'] : true,
						'ai'          => false,
						'condition'   => ! empty( $rbelad_values['condition'] ) ? $rbelad_values['condition'] : array(),
					)
				);
			...
			case 'icon_simple':
				$this->add_control(
					! empty( $rbelad_values['id'] )
						? $rbelad_values['id']
						: $rbelad_key,
					array(
						'label'            => ! empty( $rbelad_values['label'] )
							? $rbelad_values['label']
							: esc_html__( 'Icon', 'rb-addons-for-elementor' ),

						'type'             => Controls_Manager::ICONS,

						'fa4compatibility' => ! empty( $rbelad_values['fa4compatibility'] )
							? $rbelad_values['fa4compatibility']
							: '',
			...
			case 'slider':
				...
				// Units.
				$rbelad_size_units = ! empty( $rbelad_values['size_units'] )
			...
			case 'select_link_type':
				require RBELAD_CONTROLS_CONTENT_PATH . 'select-link.php';
				break;
			 ...
			 */
			case 'rating_icon':
				require RBELAD_CONTROLS_CONTENT_PATH . 'rating-icon.php';
				break;
		}
	}
}
