...
// Text Item Class.
$this->add_render_attribute( 'text_attr', 'class', 'rbelad-dual-text-item-text' );

// Text Highlight Class.
$this->add_render_attribute( 'text_highlight', 'class', 'rbelad-dual-text-item-text rbelad-dual-text-item-highlight' );

// Title Repeater.
$title_item          = ''; // phpcs:disable WordPress.NamingConventions.PrefixAllGlobals
$text_highlight_attr = $this->get_render_attribute_string( 'text_highlight' ); // phpcs:disable WordPress.NamingConventions.PrefixAllGlobals
$text_attr           = $this->get_render_attribute_string( 'text_attr' ); // phpcs:disable WordPress.NamingConventions.PrefixAllGlobals
foreach ( $settings['rbelad_dual_text_general_content_title_repeater'] as $item ) {
	if ( ! empty( $item['rbelad_dual_text_general_content_title_text'] ) ) {
		...
		if ( 'yes' === $item['rbelad_dual_text_general_content_title_highlight'] ) {
			...
			$title_item .= sprintf( '<span %1$s>%2$s</span>', $text_attr, $title_text );
		}
	}
}
...
$html_tag  = Utils::validate_html_tag( $settings['rbelad_dual_text_general_content_html_tag'] ); // phpcs:disable WordPress.NamingConventions.PrefixAllGlobals
...
if ( ! empty( $settings['rbelad_dual_text_general_content_link']['url'] ) ) {
	$html = sprintf( '<%1$s %2$s><a %3$s>%4$s</a></%1$s>', $html_tag, $html_attr, $link_attr, $title_item ); // phpcs:disable WordPress.NamingConventions.PrefixAllGlobals
} else {
	$html = sprintf( '<%1$s %2$s>%3$s</%1$s>', $html_tag, $html_attr, $title_item ); // phpcs:disable WordPress.NamingConventions.PrefixAllGlobals
}
