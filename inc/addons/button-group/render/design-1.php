...
// HTML Class
$this->add_render_attribute( 'btn_wrapper', 'class', 'rb-button-group' );

// Button Loop
$btn_item_array = [];
$btn_text = '';
foreach ( $settings['btn_group_repeater'] as $index => $item ) {	
	...
	if ( 'custom' === $item['link_type'] && ! empty ( $item['custom_link']['url'] ) ) {
		// Add dynamic class per item
		$this->add_link_attributes( $item_key, $item['custom_link'] );
		$this->add_render_attribute( $item_key, 'class', 'rb-btn-item' );
	...
	elseif ( 'page' === $item['link_type'] ) {
		// Add dynamic class per item
		$this->add_render_attribute( $item_key, 'href', esc_url( get_permalink( $settings['page_link'] ) ) );
