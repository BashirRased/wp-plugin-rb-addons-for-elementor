<?php
/**
 * Appsero SDK integration file.
 *
 * Handles plugin updater, insights, and analytics.
 *
...
class License {
	 ...
	 *
	 * @param string $rbelad_key add.
	 *
	 * @since 1.3.0
	 *
	 * @return License
	 */
	public function set_option_key( $rbelad_key ) {
		$this->option_key = $rbelad_key;
	 ...
	 *
	 * @param array $rbelad_args add.
	 *
	 * @return void
	 */
	public function add_settings_page( $rbelad_args = array() ) {
		$defaults = array(
		...
		$this->menu_args = wp_parse_args( $rbelad_args, $defaults );
				...
				<div class="appsero-license-details">
					<p>
						...
						printf(
							wp_kses_post(
								$this->client->__trans(
									'Activate <strong>%s</strong> by your license key to get professional support and automatic update from your WordPress dashboard.'
								)
							),
					...
					<form method="post" novalidate="novalidate" spellcheck="false">
						...
						<div class="license-input-fields">
							...
							<button type="submit" name="submit" class="<?php echo esc_attr( 'deactive' === $action ? 'deactive-button' : '' ); ?>">
								...
								echo esc_html(
									'active' === $action
										? $this->client->__trans( 'Activate License' )
										: $this->client->__trans( 'Deactivate License' )
								);
								?>
							</button>
						</div>
	...
	/**
	 * Check if the license is valid by a specific option.
	 *
	 * @param string $option License field/option to check.
	 ...
	 *
	 * @param string $license License key value.
	 * @return void
	 */
	private function show_active_license_info( $license ) {
		...
		<div class="active-license-info">
			<div class="single-license-info">
				...
				<?php } else { ?>
					<?php
					$remaining        = absint( $license['remaining'] ?? 0 );
					$activation_limit = absint( $license['activation_limit'] ?? 0 );
					?>

					<p class="<?php echo esc_attr( $remaining ? '' : 'occupied' ); ?>">
						<?php
						echo esc_html(
							sprintf(
								$this->client->__trans( '%1$d out of %2$d' ),
								$remaining,
								$activation_limit
							)
						);
			...
			<div class="single-license-info">
				...
				<?php
				$expiry_days = $license['expiry_days'] ?? false;

				if ( false !== $expiry_days ) {
					$expiry_days = absint( $expiry_days );
					$occupied    = $expiry_days > 21 ? '' : 'occupied';

					echo '<p class="' . esc_attr( $occupied ) . '">';
					echo esc_html( $expiry_days );
					echo ' ' . esc_html__( 'days', 'rb-addons-for-elementor' );
					echo '</p>';
				} else {
	...
	/**
	 * Render license page card header.
	 *
	 * @param string $license License key value.
	 * @return void
	 */
	 ...
	 * @return array              API response after activation.
	 */
	 ...
	 *
	 * @param string              $action  Action name (e.g. active, deactive).
	 * @param array<string,mixed> $license License information.
	 * @return string Processed license key.
	 */
	private function get_input_license_value( $action, $license ) {
		...
		if ( 'deactive' === $action && isset( $license['key'] ) ) {
			$rbelad_key_length = strlen( $license['key'] );

			return str_pad(
				substr( $license['key'], 0, (int) ( $rbelad_key_length / 2 ) ),
				$rbelad_key_length,
				'*'
			);
		}
