...
class Insights {
	 ...
	 * @return bool
	 */
	private function is_optin_request(): bool {
		if ( ! isset( $_GET['appsero_nonce'] ) ) {
			return false;
		}

		$nonce = sanitize_text_field( wp_unslash( $_GET['appsero_nonce'] ) );

		if ( ! wp_verify_nonce( $nonce, 'appsero_tracker_optin' ) ) {
			return false;
		}

		$optin = sanitize_text_field(
			wp_unslash( $_GET[ $this->client->slug . '_tracker_optin' ] ?? '' )
		);
	 ...
	 * @return bool
	 */
	private function is_optout_request(): bool {
		if ( ! isset( $_GET['appsero_nonce'] ) ) {
			return false;
		}
		...
		if ( ! wp_verify_nonce( $nonce, 'appsero_tracker_optout' ) ) {
			return false;
		}

		$optout = sanitize_text_field(
			wp_unslash( $_GET[ $this->client->slug . '_tracker_optout' ] ?? '' )
		);
	...
	public function add_weekly_schedule( $schedules ) {
		$schedules['weekly'] = array(
			...
			'display'  => __( 'Once Weekly', 'rb-addons-for-elementor' ),
		);
	...
	public function uninstall_reason_submission() {
		...
		$data['reason_id'] = sanitize_text_field(
			wp_unslash( $_POST['reason_id'] )
		);

		$data['reason_info'] = isset( $_POST['reason_info'] )
			? sanitize_text_field(
				wp_unslash( $_POST['reason_info'] )
			)
			: '';
				...
				<div class="wd-dr-modal-body">
							...
							<li data-placeholder="<?php echo esc_attr( $reason['placeholder'] ); ?>">
								<label>
									...
									<div class="wd-de-reason-icon">
										<?php
										echo rbelad_kses_intermediate( $reason['icon'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
										?>
									</div>
							...
							<?php foreach ( $custom_reasons as $reason ) { ?>
								<li data-placeholder="<?php echo esc_attr( $reason['placeholder'] ); ?>" data-customreason="true">
									<label>
										<input type="radio" name="selected-reason" value="<?php echo esc_attr( $reason['id'] ); ?>">
										<div class="wd-de-reason-icon">
											<?php echo wp_kses_post( $reason['icon'] ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
										</div>
										<div class="wd-de-reason-text"><?php echo wp_kses_post( $reason['text'] ); ?></div>
									</label>
								</li>
					...
					<p class="wd-dr-modal-reasons-bottom">
						...
						printf(
							wp_kses_post(
								$this->client->__trans(
									'We share your data with <a href="%1$s" target="_blank">Appsero</a> to troubleshoot problems &amp; make product improvements. <a href="%2$s" target="_blank">Learn more</a> about how Appsero handles your data.'
								)
							),
	 ...
	 *
	 * @param string $new_name add.
	 * @param object $new_theme add.
	 * @param object $old_theme add.
	 *
	 * @return void
	 */
